...
#include <aspect/particle/integrator/rk_2_first_order_time.h>
...
namespace aspect
{
  namespace Particle
  {
    ...
    {
      template <int dim>
      RK2FirstOrderTime<dim>::RK2FirstOrderTime()
        :
        ...
        for (typename ParticleHandler<dim>::particle_iterator it = begin_particle;
             it != end_particle; ++it, ++velocity, ++old_velocity)
          {
            ...
            if (integrator_substep == 0)
              ...
              {
                it->set_location(loc0[particle_id] + dt * (*old_velocity));
              }
            else
              {
                Assert(false,
      ...
      template <int dim>
      bool
      RK2FirstOrderTime<dim>::new_integration_step()
      {
      ...
      template <int dim>
      std::size_t
      RK2FirstOrderTime<dim>::get_data_size() const
      {
      ...
      template <int dim>
      const void *
      RK2FirstOrderTime<dim>::read_data(const typename ParticleHandler<dim>::particle_iterator &particle,
                                        const void *data)
      {
        // If integration is finished, we do not need to transfer integrator
        // data to other processors, because it will be deleted soon anyway.
      ...
      template <int dim>
      void *
      RK2FirstOrderTime<dim>::write_data(const typename ParticleHandler<dim>::particle_iterator &particle,
                                         void *data) const
      {
        // If integration is finished, we do not need to transfer integrator
        // data to other processors, because it will be deleted soon anyway.
...
namespace aspect
{
  namespace Particle
  {
    namespace Integrator
    {
      ASPECT_REGISTER_PARTICLE_INTEGRATOR(RK2FirstOrderTime,
                                          "rk2 first order time",
                                          "Second Order Runge Kutta integrator using a first order time stepping scheme"
                                          "$y_{n+1} = y_n + \\Delta t\\, v(t_{n}, y_{n} + \\frac{1}{2} k_1)$ "
