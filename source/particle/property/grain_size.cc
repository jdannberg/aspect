    ...
    namespace Property
    {
	  ...
	  GrainSize<dim>::initialize ()
      {
        ...
        AssertThrow(Plugins::plugin_type_matches<const MaterialModel::GrainSize<dim>>(this->get_material_model()),
                    ExcMessage("This particle property only makes sense in combination with the grain_size material model."));
      }
                                                       ...
                                                       std::vector<double> &data) const
      {
        ...
        data.push_back(this->get_initial_composition_manager().initial_composition(position,this->introspection().compositional_index_for_name("grain_size")));
      }
	  ...
	  GrainSize<dim>::get_property_information() const
      {
        ...
        // The grain_size field always exists in models with the grain size material model.
        property_information.emplace_back("grain_size",1);
